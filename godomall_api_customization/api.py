            ...
            for order in order_list:
                ...
                if frappe.db.exists('Godomall Order',order.find("orderNo").text.strip()):
                    ...
                    for order_delivery in order_delivery_list:
                        order_delivery_doc = frappe.get_doc('Godomall Order Delivery Data',cur_order_no + '-' + order_delivery.find('sno').text.strip())
                        if order_delivery.find('sno'):
                            order_delivery_doc.order_sno = cur_order_no + '-' + order_delivery.find('sno').text.strip()
                            order_delivery_doc.order_no = cur_order_no
                            order_delivery_doc.sno =  order_delivery.find('sno').text.strip()
                        if order_delivery.find('scmNo'):
                            order_delivery_doc.scm_no =  order_delivery.find('scmNo').text.strip()
                        if order_delivery.find('commission'):
                            order_delivery_doc.commission =  float(order_delivery.find('commission').text.strip())
                        if order_delivery.find('scmAdjustNo'):
                            order_delivery_doc.scm_adjust_no =  order_delivery.find('scmAdjustNo').text.strip()
                        if order_delivery.find('scmAdjustAfterNo'):
                            order_delivery_doc.scm_adjust_after_no =  order_delivery.find('scmAdjustAfterNo').text.strip()
                        if order_delivery.find('deliveryCharge'):
                            order_delivery_doc.delivery_charge =  float(order_delivery.find('deliveryCharge').text.strip())
                        if order_delivery.find('deliveryPolicyCharge'):
                            order_delivery_doc.delivery_policy_charge =  float(order_delivery.find('deliveryPolicyCharge').text.strip())
                        if order_delivery.find('deliveryAreaCharge'):
                            order_delivery_doc.delivery_area_charge =  float(order_delivery.find('deliveryAreaCharge').text.strip())
                        if order_delivery.find('divisionDeliveryUseDeposit'):
                            order_delivery_doc.division_delivery_use_deposit =  float(order_delivery.find('divisionDeliveryUseDeposit').text.strip())
                        if order_delivery.find('divisionDeliveryUseMileage'):
                            order_delivery_doc.division_delivery_use_mileage =  float(order_delivery.find('divisionDeliveryUseMileage').text.strip())
                        if order_delivery.find('divisionDeliveryCharge'):
                            order_delivery_doc.division_delivery_charge =  float(order_delivery.find('divisionDeliveryCharge').text.strip())
                        if order_delivery.find('divisionMemberDeliveryDcPrice'):
                            order_delivery_doc.division_member_melivery_dc_price =  float(order_delivery.find('divisionMemberDeliveryDcPrice').text.strip())
                        if order_delivery.find('deliveryInsuranceFee'):
                            order_delivery_doc.delivery_insurance_fee =  float(order_delivery.find('deliveryInsuranceFee').text.strip())
                        if order_delivery.find('deliveryFixFl'):
                            order_delivery_doc.delivery_fix_fl =  order_delivery.find('deliveryFixFl').text.strip()
                        if order_delivery.find('deliveryWeightInfo'):
                            order_delivery_doc.delivery_weight_info =  order_delivery.find('deliveryWeightInfo').text.strip()
                        if order_delivery.find('overseasDeliveryPolicy'):
                            order_delivery_doc.overseas_delivery_policy =  order_delivery.find('overseasDeliveryPolicy').text.strip()
                        if order_delivery.find('deliveryCollectFl'):
                            order_delivery_doc.delivery_collect_fl =  order_delivery.find('deliveryCollectFl').text.strip()
                        if order_delivery.find('deliveryCollectPrice'):
                            order_delivery_doc.delivery_collect_price =  float(order_delivery.find('deliveryCollectPrice').text.strip())
                        if order_delivery.find('deliveryWholeFreePrice'):
                            order_delivery_doc.delivery_whole_free_price =  float(order_delivery.find('deliveryWholeFreePrice').text.strip())
                        if order_delivery.find('statisticsOrderFl'):
                            order_delivery_doc.statistics_order_fl =  order_delivery.find('statisticsOrderFl').text.strip()
                        order_delivery_doc.save()
                    ...
                    for order_info_data in order_info_data_list:
                        order_info_doc = frappe.get_doc('Godomall Order Info Data',cur_order_no +'-'+ order_seq[idx]['idx'])
                        if order_info_data.find('orderName'):	order_info_doc.order_name = order_info_data.find('orderName').text.strip()
                        if order_info_data.find('orderEmail'):	order_info_doc.order_email= order_info_data.find('orderEmail').text.strip()
                        if order_info_data.find('orderCellPhone'):	order_info_doc.order_cell_phone= order_info_data.find('orderCellPhone').text.strip()
                        if order_info_data.find('orderZipcode'):	order_info_doc.order_zipcode= order_info_data.find('orderZipcode').text.strip()
                        if order_info_data.find('orderZonecode'):	order_info_doc.order_zonecode= order_info_data.find('orderZonecode').text.strip()
                        if order_info_data.find('orderState'):	order_info_doc.order_state= order_info_data.find('orderState').text.strip()
                        if order_info_data.find('orderCity'):	order_info_doc.order_city= order_info_data.find('orderCity').text.strip()
                        if order_info_data.find('orderAddress'):	order_info_doc.order_address= order_info_data.find('orderAddress').text.strip()
                        if order_info_data.find('orderAddressSub'):	order_info_doc.order_address_sub= order_info_data.find('orderAddressSub').text.strip()
                        if order_info_data.find('receiverName'):	order_info_doc.receiver_name= order_info_data.find('receiverName').text.strip()
                        if order_info_data.find('receiverCellPhone'):	order_info_doc.receiver_cell_phone= order_info_data.find('receiverCellPhone').text.strip()
                        if order_info_data.find('receiverUseSafeNumberFl'):	order_info_doc.receiver_use_safe_number_fl= order_info_data.find('receiverUseSafeNumberFl').text.strip()
                        if order_info_data.find('receiverSafeNumber'):	order_info_doc.receiver_safe_number= order_info_data.find('receiverSafeNumber').text.strip()
                        if order_info_data.find('receiverSafeNumberDt'):	order_info_doc.receiver_safe_number_dt= order_info_data.find('receiverSafeNumberDt').text.strip()
                        if order_info_data.find('receiverZipcode'):	order_info_doc.receiver_zipcode= order_info_data.find('receiverZipcode').text.strip()
                        if order_info_data.find('receiverZonecode'):	order_info_doc.receiver_zonecode= order_info_data.find('receiverZonecode').text.strip()
                        if order_info_data.find('receiverCountry'):	order_info_doc.receiver_country= order_info_data.find('receiverCountry').text.strip()
                        if order_info_data.find('receiverState'):	order_info_doc.receiver_state= order_info_data.find('receiverState').text.strip()
                        if order_info_data.find('receiverCity'):	order_info_doc.receiver_city= order_info_data.find('receiverCity').text.strip()
                        if order_info_data.find('receiverAddress'):	order_info_doc.receiver_address= order_info_data.find('receiverAddress').text.strip()
                        if order_info_data.find('receiverAddressSub'):	order_info_doc.receiver_address_sub= order_info_data.find('receiverAddressSub').text.strip()
                        if order_info_data.find('deliveryVisit'):	order_info_doc.delivery_visit= order_info_data.find('deliveryVisit').text.strip()
                        if order_info_data.find('orderinfoCd'):	order_info_doc.orderinfo_cd= order_info_data.find('orderinfoCd').text.strip()
                        if order_info_data.find('gitMessage'):	order_info_doc.gift_message= order_info_data.find('gitMessage').text.strip()
                    ...
                    for order_goods_data in order_goods_data_list:
                        ...
                        order_goods_doc = frappe.get_doc('Godomall Order Goods Data',cur_order_no + '-' +order_goods_data.find('sno').text.strip())
                        if order_goods_data.find('sno'):	order_goods_doc.sno= order_goods_data.find('sno').text.strip()
                        if order_goods_data.find('orderNo'):	order_goods_doc.order_no= order_goods_data.find('orderNo').text.strip()
                        if order_goods_data.find('mailSno'):	order_goods_doc.mall_sno= order_goods_data.find('mailSno').text.strip()
                        if order_goods_data.find('apiOrderGoodsNo'):	order_goods_doc.api_order_goods_no= order_goods_data.find('apiOrderGoodsNo').text.strip()
                        if order_goods_data.find('orderCd'):	order_goods_doc.order_cd= order_goods_data.find('orderCd').text.strip()
                        if order_goods_data.find('orderGroupCd'):	order_goods_doc.order_group_cd= order_goods_data.find('orderGroupCd').text.strip()
                        if order_goods_data.find('eventCd'):	order_goods_doc.event_sno= order_goods_data.find('eventCd').text.strip()
                        if order_goods_data.find('orderStatus'):	order_goods_doc.order_status= order_goods_data.find('orderStatus').text.strip()
                        if order_goods_data.find('orderDeliverySno'):	order_goods_doc.order_delivery_sno= order_goods_data.find('orderDeliverySno').text.strip()
                        if order_goods_data.find('invoiceCompanySno'):	order_goods_doc.invoice_company_sno= order_goods_data.find('invoiceCompanySno').text.strip()
                        if order_goods_data.find('invoiceNo'):	order_goods_doc.invoice_no= order_goods_data.find('invoiceNo').text.strip()
                        if order_goods_data.find('scmNo'):	order_goods_doc.scm_no= order_goods_data.find('scmNo').text.strip()
                        if order_goods_data.find('purchaseNo'):	order_goods_doc.purchase_no= order_goods_data.find('purchaseNo').text.strip()
                        if order_goods_data.find('commission'):	order_goods_doc.commission= order_goods_data.find('commission').text.strip()
                        if order_goods_data.find('scmAdjustAfterNo'):	order_goods_doc.scm_adjust_after_no= order_goods_data.find('scmAdjustAfterNo').text.strip()
                        if order_goods_data.find('goodsType'):	order_goods_doc.goods_type= order_goods_data.find('goodsType').text.strip()
                        if order_goods_data.find('timeSaleFl'):	order_goods_doc.time_sale_fl= order_goods_data.find('timeSaleFl').text.strip()
                        if order_goods_data.find('parentMustFl'):	order_goods_doc.parent_must_fl= order_goods_data.find('parentMustFl').text.strip()
                        if order_goods_data.find('parentGoodsNo'):	order_goods_doc.parent_goods_no= order_goods_data.find('parentGoodsNo').text.strip()
                        if order_goods_data.find('goodsNo'):	order_goods_doc.goods_no= order_goods_data.find('goodsNo').text.strip()
                        if order_goods_data.find('listImageData'):	order_goods_doc.list_image_data= order_goods_data.find('listImageData').text.strip()
                        if order_goods_data.find('goodsCd'):	order_goods_doc.goods_cd= order_goods_data.find('goodsCd').text.strip()
                        if order_goods_data.find('goodsModelNo'):	order_goods_doc.goods_model_no= order_goods_data.find('goodsModelNo').text.strip()
                        if order_goods_data.find('goodsNm'):	order_goods_doc.goods_nm= order_goods_data.find('goodsNm').text.strip()
                        if order_goods_data.find('goodsNmStandard'):	order_goods_doc.goods_nm_standard= order_goods_data.find('goodsNmStandard').text.strip()
                        if order_goods_data.find('goodsCnt'):	order_goods_doc.goods_cnt= int(order_goods_data.find('goodsCnt').text.strip())
                        if order_goods_data.find('goodsPrice'):	order_goods_doc.goods_price= float(order_goods_data.find('goodsPrice').text.strip())
                        if order_goods_data.find('divisionUseDeposit'):	order_goods_doc.division_use_deposit= float(order_goods_data.find('divisionUseDeposit').text.strip())
                        if order_goods_data.find('divisionUseMileage'):	order_goods_doc.division_use_mileage= float(order_goods_data.find('divisionUseMileage').text.strip())
                        if order_goods_data.find('divisionGoodsDeliveryUseDeposit'):	order_goods_doc.division_goods_delivery_use_deposit= float(order_goods_data.find('divisionGoodsDeliveryUseDeposit').text.strip())
                        if order_goods_data.find('divisionGoodsDeliveryUseMileage'):	order_goods_doc.division_goods_delivery_use_mileage= float(order_goods_data.find('divisionGoodsDeliveryUseMileage').text.strip())
                        if order_goods_data.find('divisionCouponOrderDcPrice'):	order_goods_doc.division_coupon_order_dc_price= float(order_goods_data.find('divisionCouponOrderDcPrice').text.strip())
                        if order_goods_data.find('divisionCouponOrderMileage'):	order_goods_doc.division_coupon_order_mileage= float(order_goods_data.find('divisionCouponOrderMileage').text.strip())
                        if order_goods_data.find('addGoodsPrice'):	order_goods_doc.add_goods_price= float(order_goods_data.find('addGoodsPrice').text.strip())
                        if order_goods_data.find('optionPrice'):	order_goods_doc.option_price= float(order_goods_data.find('optionPrice').text.strip())
                        if order_goods_data.find('optionCostPrice'):	order_goods_doc.option_cost_price= float(order_goods_data.find('optionCostPrice').text.strip())
                        if order_goods_data.find('optionTextPrice'):	order_goods_doc.option_text_price= float(order_goods_data.find('optionTextPrice').text.strip())
                        if order_goods_data.find('fixedPrice'):	order_goods_doc.fixed_price= float(order_goods_data.find('fixedPrice').text.strip())
                        if order_goods_data.find('costPrice'):	order_goods_doc.cost_price=float( order_goods_data.find('costPrice').text.strip())
                        if order_goods_data.find('goodsDcPrice'):	order_goods_doc.goods_dc_price= float(order_goods_data.find('goodsDcPrice').text.strip())
                        if order_goods_data.find('memberDcPrice'):	order_goods_doc.member_dc_price= float(order_goods_data.find('memberDcPrice').text.strip())
                        if order_goods_data.find('memberOverlapDcPrice'):	order_goods_doc.member_overlap_dc_price= float(order_goods_data.find('memberOverlapDcPrice').text.strip())
                        if order_goods_data.find('couponGoodsDcPrice'):	order_goods_doc.coupon_goods_dc_price= float(order_goods_data.find('couponGoodsDcPrice').text.strip())
                        if order_goods_data.find('timeSalePrice'):	order_goods_doc.time_sale_price= float(order_goods_data.find('timeSalePrice').text.strip())
                        if order_goods_data.find('brandBankSalePrice'):	order_goods_doc.brand_bank_sale_price= float(order_goods_data.find('brandBankSalePrice').text.strip())
                        if order_goods_data.find('myappDcPrice'):	order_goods_doc.myapp_dc_price= float(order_goods_data.find('myappDcPrice').text.strip())
                        if order_goods_data.find('goodsDeliveryCollectPrice'):	order_goods_doc.goods_delivery_collect_price= float(order_goods_data.find('goodsDeliveryCollectPrice').text.strip())
                        if order_goods_data.find('goodsMileage'):	order_goods_doc.goods_mileage= float(order_goods_data.find('goodsMileage').text.strip())
                        if order_goods_data.find('memberMileage'):	order_goods_doc.member_mileage= float(order_goods_data.find('memberMileage').text.strip())
                        if order_goods_data.find('couponGoodsMileage'):	order_goods_doc.coupon_goods_mileage= float(order_goods_data.find('couponGoodsMileage').text.strip())
                        if order_goods_data.find('goodsDeliveryCollectPrice'):	order_goods_doc.goods_delivery_collect_fl= order_goods_data.find('goodsDeliveryCollectPrice').text.strip()
                        if order_goods_data.find('minusDepositFl'):	order_goods_doc.minus_deposit_fl= order_goods_data.find('minusDepositFl').text.strip()
                        if order_goods_data.find('minusRestoreDepositFl'):	order_goods_doc.minus_restore_deposit_fl= order_goods_data.find('minusRestoreDepositFl').text.strip()
                        if order_goods_data.find('minusMileageFl'):	order_goods_doc.minus_mileage_fl= order_goods_data.find('minusMileageFl').text.strip()
                        if order_goods_data.find('minusRestoreMileageFl'):	order_goods_doc.minus_restore_mileage_fl= order_goods_data.find('minusRestoreMileageFl').text.strip()
                        if order_goods_data.find('plusMileageFl'):	order_goods_doc.plus_mileage_fl= order_goods_data.find('plusMileageFl').text.strip()
                        if order_goods_data.find('plusRestoreMileageFl'):	order_goods_doc.plus_restore_mileage_fl= order_goods_data.find('plusRestoreMileageFl').text.strip()
                        if order_goods_data.find('minusStockFl'):	order_goods_doc.minus_stock_fl= order_goods_data.find('minusStockFl').text.strip()
                        if order_goods_data.find('minusRestoreStockFl'):	order_goods_doc.minus_restore_stock_fl= order_goods_data.find('minusRestoreStockFl').text.strip()
                        if order_goods_data.find('optionSno'):	order_goods_doc.option_sno= order_goods_data.find('optionSno').text.strip()
                        if order_goods_data.find('optionInfo'):	order_goods_doc.option_info= order_goods_data.find('optionInfo').text.strip()
                        if order_goods_data.find('optionTextInfo'):	order_goods_doc.option_text_info= order_goods_data.find('optionTextInfo').text.strip()
                        if order_goods_data.find('cateAllCd'):	order_goods_doc.cate_all_cd= order_goods_data.find('cateAllCd').text.strip()
                        if order_goods_data.find('hscode'):	order_goods_doc.hscode= order_goods_data.find('hscode').text.strip()
                        if order_goods_data.find('cancelDt'):	order_goods_doc.cancel_dt= order_goods_data.find('cancelDt').text.strip()
                        if order_goods_data.find('paymentDt'):	order_goods_doc.payment_dt= order_goods_data.find('paymentDt').text.strip()
                        if order_goods_data.find('invoiceDt'):	order_goods_doc.invoice_dt= order_goods_data.find('invoiceDt').text.strip()
                        if order_goods_data.find('deliveryDt'):	order_goods_doc.delivery_dt= order_goods_data.find('deliveryDt').text.strip()
                        if order_goods_data.find('deliveryCompleteDt'):	order_goods_doc.delivery_complete_dt= order_goods_data.find('deliveryCompleteDt').text.strip()
                        if order_goods_data.find('finishDt'):	order_goods_doc.finish_dt= order_goods_data.find('finishDt').text.strip()
                        if order_goods_data.find('mileageGiveDt'):	order_goods_doc.mileage_give_dt= order_goods_data.find('mileageGiveDt').text.strip()
                        if order_goods_data.find('checkoutData'):	order_goods_doc.checkout_data= order_goods_data.find('checkoutData').text.strip()
                        if order_goods_data.find('statisticsOrderFl'):	order_goods_doc.statistics_order_fl= order_goods_data.find('statisticsOrderFl').text.strip()
                        if order_goods_data.find('statisticsGoodsFl'):	order_goods_doc.statistics_goods_fl= order_goods_data.find('statisticsGoodsFl').text.strip()
                        if order_goods_data.find('deliveryMethodFl'):	order_goods_doc.delivery_method_fl= order_goods_data.find('deliveryMethodFl').text.strip()
                        if order_goods_data.find('enuri'):	order_goods_doc.enuri= order_goods_data.find('enuri').text.strip()
                        if order_goods_data.find('goodsDiscountInfo'):	order_goods_doc.goods_discount_info= order_goods_data.find('goodsDiscountInfo').text.strip()
                        if order_goods_data.find('goodsMileageAddInfo'):	order_goods_doc.goods_mileage_add_info= order_goods_data.find('goodsMileageAddInfo').text.strip()
                        if order_goods_data.find('inflow'):	order_goods_doc.inflow= order_goods_data.find('inflow').text.strip()
                        if order_goods_data.find('linkMainTheme'):	order_goods_doc.link_main_theme= order_goods_data.find('linkMainTheme').text.strip()
                        if order_goods_data.find('visitAddress'):	order_goods_doc.visit_address= order_goods_data.find('visitAddress').text.strip()
                        if order_goods_data.find('dpxDeliveryType'):	order_goods_doc.dpx_delivery_type= order_goods_data.find('dpxDeliveryType').text.strip()
                        if order_goods_data.find('dawnInfoType'):	order_goods_doc.dawn_info_type= order_goods_data.find('dawnInfoType').text.strip()
                        if order_goods_data.find('dawnInfoMemo'):	order_goods_doc.dawn_info_memo= order_goods_data.find('dawnInfoMemo').text.strip()
                        if order_goods_data.find('dawnInfoAlram'):	order_goods_doc.dawn_info_alram= order_goods_data.find('dawnInfoAlram').text.strip()
                        if order_goods_data.find('requestBagFl'):	order_goods_doc.request_bag_fl= order_goods_data.find('requestBagFl').text.strip()
                        if order_goods_data.find('goodsVolume'):	order_goods_doc.goods_volume= order_goods_data.find('goodsVolume').text.strip()
                        if order_goods_data.find('couponMileageFl'):	order_goods_doc.coupon_mileage_fl= order_goods_data.find('couponMileageFl').text.strip()
                        if order_goods_data.find('deliveryBundleFl'):	order_goods_doc.delivery_bundle_fl= order_goods_data.find('deliveryBundleFl').text.strip()
                        if order_goods_data.find('deliveryDueDate'):	order_goods_doc.delivery_due_date= order_goods_data.find('deliveryDueDate').text.strip()
                        if order_goods_data.find('easypayScmReceiptFl'):	order_goods_doc.easypay_scm_receipt_fl= order_goods_data.find('easypayScmReceiptFl').text.strip()
